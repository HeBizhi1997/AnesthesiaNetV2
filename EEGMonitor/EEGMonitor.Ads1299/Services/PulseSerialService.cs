...
/// <summary>
/// HKG-07D pulse sensor — 5-byte passive push protocol:
///   [0]0xF0 [1]0xC0 [2]BPM-hi [3]BPM-lo [4]checksum=(F0+C0+hi+lo)&amp;0xFF
/// BPM = (hi&lt;&lt;8)|lo; 0 = no signal. Frame pushed every ~9 s at 19200 baud.
/// </summary>
public sealed class PulseSerialService : IDisposable
{
    ...
    public PulseSerialService(ILogger<PulseSerialService> logger) => _logger = logger;
    ...
    public bool Connect(string portName)
    {
        ...
        try
        {
            ...
            _port.Open();
            _logger.LogInformation("Pulse port {Port} opened", portName);
            ConnectionStatusChanged?.Invoke($"脉搏已连接 {portName}");
            return true;
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Failed to open pulse port {Port}", portName);
            return false;
        }
    }
    ...
    private void OnDataReceived(object sender, Ports.SerialDataReceivedEventArgs e)
    {
        ...
        catch (Exception ex) { _logger.LogWarning(ex, "Error reading pulse data"); }
    }
    ...
    private void ProcessBuffer()
    {
        while (_buffer.Count >= FRAME_SIZE)
        {
            ...
            if (_buffer.Count < FRAME_SIZE) return;
            if (_buffer[1] != HEADER1) { _buffer.RemoveAt(0); continue; }

            byte mlh = _buffer[2], mll = _buffer[3], cksum = _buffer[4];
            byte expected = (byte)((HEADER0 + HEADER1 + mlh + mll) & 0xFF);
            if (cksum != expected) { _buffer.RemoveAt(0); continue; }
